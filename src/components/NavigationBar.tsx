...
import { Search, Grid2X2, Building2, MessageSquareQuote, Users, Bell, Settings, Menu, User, ChevronDown } from "lucide-react";
            ...
            <Tooltip delayDuration={1000}>
              <TooltipTrigger asChild>
                <Link to="/profile" className="relative block">
                  <button className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-300 hover:bg-white/10 ${isActive('/profile') ? 'bg-white/10' : ''}`}>
                    <User className={`w-6 h-6 transition-colors duration-300 ${isActive('/profile') ? 'text-white' : 'text-[#9CA3AF] hover:text-white'}`} />
                  </button>
                  {isActive('/profile') && (
                    <div className="w-1 h-1 absolute -bottom-2 left-1/2 transform -translate-x-1/2 bg-[#2cff3a] rounded-full" />
                  )}
              ...
              <TooltipContent side="right" className="bg-black text-white border-none">
                <p className="font-inter">Marquise Tan</p>
              </TooltipContent>
            </Tooltip>
          </div>
        </div>
